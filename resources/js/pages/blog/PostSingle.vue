...
<div class="min-h-screen flex flex-col">
    ...
    <div class="container mx-auto flex flex-wrap py-6">
        ...
        <section class="w-full md:w-2/3 flex flex-col px-3">
            ...
            <article class="flex flex-col w-full shadow my-4">
                ...
                <div class="bg-white flex flex-col justify-start p-6">
                    ...
                    <p class="text-sm pb-8">
                        ...
                        , Published on {{createdAt(post.created_at)}}
                    </p>

                    <!-- Post body (Editor.js object) will be rendered here -->
                    <div v-html="post.post"></div>
                    <div>
                        <h1 class="text-3xl font-bold py-4">Comments</h1>
                        <div v-for="comment in commentsInfo" :key="comment.id" v-if="commentsInfo">
                            <p class="text-base my-3">
                                {{comment.desc}}
                            </p>
                            <div class="text-right text-gray-400">
                                <label class="text-sm">
                                    {{comment.user.name}},  Published on {{createdAt(comment.created_at)}}
                                </label>
                            </div>
                        </div>
            ...
            <div v-if="user">
                <div class="flex flex-col w-full shadow my-4">
                    <Input id="addingComment" type="textarea" v-model="data.desc" :rows="4" placeholder="Add new comment" />
                </div>
                <div class="float-left">
                    <Button size="large" @click="addComment" :loading="isAdding" :disabled="isAdding"><Icon type="md-create" /> {{isAdding ? 'Please wait...' : 'Add Comment'}}</Button>
                </div>
            </div>
...
export default {
    ...
    computed : {
        ...
        ...mapGetters(['getAllComments','setAllComments','user'])
	},
    methods: {
        createdAt (date){
        ...
        async getCommentData() {
			...
			if(res.status==200){
                this.$store.commit('setAllComments',res.data);
        ...
        async addComment(){
            ...
            if(res.status===200){
                this.s('Comment has been created successfully!');		 
                this.$router.push(`/post/${this.$route.params.slug}`);
            }else{
                if(res.status==422){
